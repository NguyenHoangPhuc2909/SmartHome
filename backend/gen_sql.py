...
DEVICES = [
    (5, 'light'), (6, 'fan'),   # Living Room
    (7, 'light'), (8, 'fan'),   # Bedroom
    (9, 'light'),                # Entrance
    (10, 'alarm'),               # Kitchen
]

def get_status(dtype, hour, temp, light, gas):
    if dtype == 'fan':
        return 1 if temp > 29.5 else 0
    if dtype == 'alarm':
        return 1 if gas > 50 else 0
    if dtype == 'light':
...
for day in range(30):
    for hour in [0,2,4,6,7,9,11,13,15,18,20,22]:
        ...
        gas = random.uniform(0, 15) # Default low gas
        
        # Thêm thỉnh thoảng rò rỉ gas để test còi
        ...
        for did, dtype in DEVICES:
            s = get_status(dtype, hour, temp, light, gas)
            rows.append(
                "({},{}, 'AI',{:.1f},{:.1f},{:.1f},{:.1f},'{}')".format(
                    did, s, temp, humi, light, gas, ts
                )
            )
